...
int main()
{
	...
	boost = Boost::load("TrainingWithAdaboost.xml");
	Mat Resized;
	Mat greyMat;
	...
	for (int i = 0; i < 1827; ++i)
	{
		...
		float response = boost->predict(testing_data.row(i)); // predict
		if (i < 1374) {
	...
	cout <<"Temps execution "<< temps <<"s"<< endl;
	cout << "Temps execution " << temps/1827 << "s" << endl;
